...
// --- DEĞİŞİKLİK: FaUserShield ikonu eklendi ---
import { FaBars, FaTimes, FaUserCircle, FaHome, FaFileAlt, FaCalendar, FaPlus, FaSignOutAlt, FaUserPlus, FaSignInAlt, FaCog, FaBullseye, FaTicketAlt, FaUserShield } from 'react-icons/fa';
import './Header.css';
          ...
          <div className="admin-menu-container">
            ...
            {isMenuOpen && (
              <div className="dropdown-menu">
                {currentUser ? (
                   <>
                    {/* --- DEĞİŞİKLİK: Admin Paneli linki sadece superadmin'e görünecek --- */}
                    {currentUser.role === 'superadmin' && (
                        <Link to="/super-admin" className="dropdown-link" onClick={closeMenu} style={{color: '#ffc107', fontWeight: 'bold'}}>
                            <FaUserShield /> Yönetim Paneli
                        </Link>
                    )}
                    ...
                    <Link to="/profil/abonelik" className="dropdown-link" onClick={closeMenu}><FaTicketAlt /> Aboneliğim</Link>
