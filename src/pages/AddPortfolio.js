...
// --- DEĞİŞİKLİK 1: Gelen prop'lar güncellendi ---
function AddPortfolio({ properties, onAddPortfolio, onUpdatePortfolio }) {
  ...
  const [formData, setFormData] = useState({
    ...
    doorCode: '', privateNote: '', isPublished: true // Varsayılan olarak yayında
  });
  ...
  useEffect(() => {
    if (isEditMode && properties) {
      // id'nin string olabileceğini varsayarak `==` veya `toString()` kullanalım
      const portfolioToEdit = properties.find(p => p.id.toString() === id);
      if (portfolioToEdit) { 
        // images dizisini virgülle ayrılmış string'e çeviriyoruz
        const currentData = { ...portfolioToEdit };
        if (Array.isArray(currentData.images)) {
          currentData.images = currentData.images.join(', ');
        }
  ...
  useEffect(() => {
    ...
    if (formData.location && formData.location.lat && !formData.address) {
       getAddressFromCoords(formData.location.lat, formData.location.lng);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Mevcut resim URL'lerini (string ise) diziye çevir
    ...
    try {
      ...
      // Eğer yeni resimler seçildiyse, mevcutları temizle ve yenilerini yükle
      if (selectedFiles.length > 0) {
        uploadedImageUrls = []; // Eski resim listesini sıfırla
      ...
      // --- DEĞİŞİKLİK 2: Düzenleme modunda onUpdatePortfolio, ekleme modunda onAddPortfolio çağrılıyor ---
      if (isEditMode) {
    ...
    return (
        <div className="add-portfolio-container">
            ...
            {/* Formun geri kalanı aynı, değişiklik yok... */}
