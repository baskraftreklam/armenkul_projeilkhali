...
function AddPortfolio({ properties, onAddPortfolio, onUpdatePortfolio }) {
  ...
  useEffect(() => {
    if (isEditMode) {
      ...
      if (portfolioToEdit) {
        setFormData({ ...portfolioToEdit, images: portfolioToEdit.images.join(', ') });
      }
    }
  ...
  useEffect(() => {
    const getAddressFromCoords = async (lat, lng) => {
      ...
      try {
        ...
        setFormData(prev => ({ ...prev, address: 'Adres alınamadı. Lütfen elle girin.' }));
      } finally {
  ...
  return (
    <div className="add-portfolio-container">
      ...
      <form onSubmit={handleSubmit} className="portfolio-form">
        ...
        {/* --- DEĞİŞİKLİK BURADA: Açıklama bölümü buraya taşındı --- */}
        <div className="form-section">
